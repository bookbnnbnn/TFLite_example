  ...
  def create_full_integer_quantization(
    ...
    Args:
      ...
      inference_input_type: Target data type of real-number input arrays. Used
        only when `is_integer_only` is True. Must be `{tf.uint8, tf.int8}`.
    ...
    if not is_integer_only:
      ...
      if inference_input_type != tf.int8 and inference_input_type != tf.uint8:
        raise ValueError('For integer only quantization, `inference_input_type`'
                         'should be tf.int8 or tf.uint8.')
      if inference_output_type != tf.int8 and inference_output_type != tf.uint8:
        raise ValueError(
            'For integer only quantization, '
            '`inference_output_type` should be tf.int8 or tf.uint8.')
